    ...
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader] = None):
        ...
        if self.meta_path.exists():
            try:
                ...
                self._meta = {"rows": {}} # init the empty one if doesn't exist
...
class ChatIngestor:
    def __init__(self,
        ...
        try:
            ...
            log.info("ChatIngestor initialized", session_id=self.session_id, temp_dir=str(self.temp_dir), faiss_dir=str(self.faiss_dir), sessionized=self.use_session)
        except Exception as e:
            log.error("Failed to initalize ChatIngestor", error=str(e))
            raise DocumentPortalException("Failed to initalize ChatIngestor", e) from e
...
class DocHandler:
    ...
    def save_pdf(self, uploaded_file) -> str:
        try:
            filename:str = os.path.basename(uploaded_file.name)
