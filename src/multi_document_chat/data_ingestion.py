...
class DocumentIngestor:
    ...
    def ingest_files(self, uploaded_files):
        try: 
            ...
            for uploaded_file in uploaded_files:
                ...
                self.log.info("File saved for ingestion", filename=uploaded_file.name)
            ...
            if not documents:
                raise DocumentPortalException("No documents loaded from uploaded files", sys)
    ...
    def _create_retriever(self, documents):
        try: 
            ...
            self.log.info("Documents split into chunks", count=len(chunks), session_id=self.session_id)
